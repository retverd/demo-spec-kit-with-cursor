...
- Верхняя граница допустимой длительности (предположение: 365 дней) — проверяется корректность периода и сообщения об ошибке при превышении.
...
- Выполнение в дату смены месяца/года или на високосный год — конец периода остается сегодняшним днем.
  - Покрытие граничных расчётов и смены даты — задачи T025 (unit).
  - Реальные прогоны против API CBR/MOEX и регрессия форматов после `--days` — задачи T022–T024 (Phase 6).
...
- **FR-003**: Значение `days` должно быть целым числом в допустимом диапазоне (предположение: 1–365); выход за пределы или неверный тип вызывает ошибку валидации без обращения к внешним сервисам.
...
- **SC-001**: Запуск `cbr` с валидным `--days` (1–365) завершается успешно и создает файл с корректным интервалом, включающим текущую дату, в 100% тестовых прогонов.
- **SC-002**: Запуск `moex-lqdt` с валидным `--days` (1–365) создает XLSX с корректным интервалом, включающим текущую дату, в 100% тестовых прогонов.
...
- **SC-004**: Для выборки контрольных значений `days` (1, 7, 365) и дат смены месяца/високосного года рассчитанные даты в логах и метаданных совпадают с ожидаемыми в 100% проверок.
- **SC-005**: Реальные прогоны CLI против API CBR и MOEX с `--days {1,7,30}` завершаются успешно, логи/метаданные содержат фактический период; фиксируются время отклика и пропускная способность без сбоев.
- **SC-006**: Регрессионные проверки существующих форматов/имен файлов для `cbr` и `moex-lqdt` после введения обязательного `--days` не выявляют расхождений с эталонными выборками (формат, структура, коды выхода).
