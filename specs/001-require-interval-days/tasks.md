...
- [X] T022 [US1] Добавлены опциональные реальные прогоны CBR (через pytest, включаются `RUN_REAL_API=1`) с фиксацией времени и проверкой структуры/метаданных Parquet: `tests/integration/test_real_api_runs.py`.
- [X] T023 [US2] Добавлены опциональные реальные прогоны MOEX (через pytest, включаются `RUN_REAL_API=1`) с фиксацией времени и проверкой XLSX (лист/заголовки/строки): `tests/integration/test_real_api_runs.py`.
- [X] T024 Регрессионная проверка артефактов (имена файлов, метаданные, структура) реализована в реальных прогонах CBR/MOEX и запускается по требованию (по умолчанию пропускается).
...
- [X] T020 [P] Синхронизированы примеры и коды выхода в `specs/001-require-interval-days/quickstart.md` с финальной реализацией (обязательный `--days`).
- [X] T021 Проведена ревизия `--help`/описаний в `src/cli/main.py`: уточнено описание обязательного `--days`, включена стандартная справка подкоманд.
