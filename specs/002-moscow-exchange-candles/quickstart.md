...
- В файле один лист с колонками: `Date, Open, High, Low, Close, Volume` и ровно `--days` строками по датам периода

## Параметры по умолчанию

- Период: последние `--days` календарных дней `[today-(days-1), ..., today]`
- Инструмент: `LQDT`
- Доска: `TQTF`
...
1. **Успешный запрос**  
   - Команда: `python -m src.cli.main moex-lqdt --days 7`  
...
- `5` — ошибка валидации (в т.ч. отсутствие/некорректный `--days`)
