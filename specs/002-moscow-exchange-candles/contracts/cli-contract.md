...
Запросить дневные свечи LQDT/TQTF за последние `--days` календарных дней через ISS‑API и сохранить результат в XLSX‑файл в текущей директории.
...
- `--days` / `-d` — длительность периода в днях (**обязательно**, целое 1–365). Конец периода — сегодня.
...
- Ровно `--days` строк по датам периода, пустые ячейки для дней без данных.
...
- `5` — ошибка валидации данных свечей (в т.ч. отсутствие/некорректный `--days`)
...
1. Рассчитать период `[today-(days-1) .. today]`.
