...
**Localization**: все пользовательские CLI‑сообщения и сообщения об ошибках для режима `moex-lqdt` выводятся на русском языке в соответствии с Принципом 6 конституции.  
...
6. **CLI‑слой (`cli/main.py`)**:  
   ...
   - Логика в режиме LQDT/TQTF:
     ...
     4. Подготовить значения для имени файла и логирования: `period_start`, `period_end`, текущая дата и время запуска, а также константы `data_source="MOEX-ISS"`, `instrument="LQDT"`, `board="TQTF"`;  
     5. Записать файл через `XLSXWriter().write_candles(records, output_dir=".")`;  
     6. Обработать ошибки и вернуть соответствующие коды выхода (можно переиспользовать те же константы или добавить отдельные для MOEX, сохранив семантику; все сообщения об ошибках должны быть на русском языке).
